...
if (isset($_GET['mid']) && $_GET['mid'] != '') {
    $delete_responce = delete_metafield($shopdata[0]['store_name'], $shopdata[0]['access_token'], 'shop', '', $_GET['mid']);
}
if (isset($_POST['submit']) && $_POST['submit'] == 'Save') {
    ...
    if (count($dataexisting) > 0) {
        foreach ($dataexisting as $mid => $data) {
            $res = update_metafield($shopdata[0]['store_name'], $shopdata[0]['access_token'], 'shop', $_GET['id'], $mid, $data);
        }
    }
    if (count($datanew) > 0) {
...
//($shopdata);
$metafields_json = get_metafield($shopdata[0]['store_name'], $shopdata[0]['access_token'], 'shop');
